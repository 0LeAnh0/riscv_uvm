...
// object.
//***************************************************************************************************************
class risc_v_master_driver_base #(type REQ = uvm_sequence_item, type RSP = uvm_sequence_item) extends uvm_driver #(REQ,RSP);

   `uvm_component_param_utils(risc_v_master_driver_base #(REQ,RSP))

   string   my_name;  // local name to be used in place of get_name()
   
   // risc_v_cfg     driver_cfg;
   integer       rsp_pkt_cnt;
   ...
   function void connect_phase(uvm_phase phase);
      ...
      //
      // if( !uvm_config_db #(risc_v_cfg)::get(this,"","TB_CONFIG",driver_cfg) ) begin
      //    `uvm_error(my_name, "Could not retrieve risc_v_cfg");
      // end
