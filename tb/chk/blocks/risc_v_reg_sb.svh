...
class risc_v_reg_sb extends uvm_scoreboard;
  ...
  virtual function void write(risc_v_tlm tr);
    ...
    // Check READs (at current instruction)
    ...
    // Update WRITE (for future instructions)
    if (tr.reg_write && rd != 0) begin
      shadow_regs[rd] = tr.wb_data;
      `uvm_info("REG_SB", $psprintf("Reg x%0d written with %h", rd, tr.wb_data), UVM_HIGH)
    end
  endfunction
